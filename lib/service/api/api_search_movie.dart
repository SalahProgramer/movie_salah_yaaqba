...
class ApiSearchMovie{
  Future<List<MovieModel>> apiSearchMovie({required String title,required int numPage}) async {
    List<MovieModel> movieList;
    if (await isConnectedWifi()) {
      try {
        var response = await http.get(Uri.parse("$baseUrl?apikey=$apikey&s=$title&page=$numPage&type=movie"),
            headers: headers);
